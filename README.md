...
- **Off-ramp (Crypto to Naira, SEND):** Deposit = **Coinbase Smart Wallet** (CDP); sweep is **Paymaster-sponsored** (no EOA gas funding). Set **COINBASE_API_KEY_NAME**, **COINBASE_API_KEY_PRIVATE_KEY**, **COINBASE_APP_ID**, and **COINBASE_BUNDLER_RPC_URL** (CDP Portal → Paymaster → Base mainnet endpoint). Add the **SEND token** and `transfer(address,uint256)` to your Paymaster allowlist (see [OFFRAMP_FLOW.md](./docs/OFFRAMP_FLOW.md)). Bank verification and Naira payouts use **Flutterwave** (all banks: OPay, Palmpay, GTBank, etc.). Use **live** API keys and do not set `FLUTTERWAVE_USE_TEST_MODE=true`. For **V3 only**, set `FLUTTERWAVE_FORCE_V3=true` and `FLUTTERWAVE_SECRET_KEY`. **Optional – NIBSS:** To use NIBSS for bank account verification (name enquiry), set **NIBSS_NAME_ENQUIRY_URL** (and **NIBSS_API_KEY** if required). If set, verification tries NIBSS first, then falls back to Flutterwave. See [docs/setup/NIBSS_VERIFICATION.md](./docs/setup/NIBSS_VERIFICATION.md). SEND is swept to: **OFFRAMP_RECEIVER_WALLET_ADDRESS** if set, else **OFFRAMP_POOL_PRIVATE_KEY** address, else liquidity pool. **Production:** Call `POST /api/offramp/process-payouts` every 5 min (cron-job.org or Vercel cron) so payouts run automatically; set **OFFRAMP_CRON_SECRET** if you protect the endpoint. Optional: `OFFRAMP_MIN_SEND_SWEEP` (default `0.01`). For **instant** payout when user sends SEND, set **CDP_WEBHOOK_SECRET** (from CDP Portal → Webhooks) and register your app URL as `https://your-domain.com/api/offramp/webhook` (see [OFFRAMP_FLOW.md](./docs/OFFRAMP_FLOW.md)). Users can cancel a pending off-ramp via the "Cancel pending and start new" button (API: `POST /api/offramp/cancel-pending`).
